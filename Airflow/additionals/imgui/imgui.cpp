...
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
    ...
    buf_local.clear();
    if (!LI_FN(OpenClipboard).cached()(NULL))
        return NULL;
    HANDLE wbuf_handle = LI_FN(GetClipboardData).cached()(CF_UNICODETEXT);
    if (wbuf_handle == NULL)
    {
        LI_FN(CloseClipboard).cached()();
        return NULL;
    }
    if (ImWchar* wbuf_global = (ImWchar*)LI_FN(GlobalLock).cached()(wbuf_handle))
    {
        int buf_len = ImTextCountUtf8BytesFromStr(wbuf_global, NULL) + 1;
        buf_local.resize(buf_len);
        ImTextStrToUtf8(buf_local.Data, buf_len, wbuf_global, NULL);
    }
    LI_FN(GlobalLock).cached()(wbuf_handle);
    LI_FN(CloseClipboard).cached()();
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
    ...
    HGLOBAL wbuf_handle = LI_FN(GlobalAlloc).cached()(GMEM_MOVEABLE, (SIZE_T)wbuf_length * sizeof(ImWchar));
    if (wbuf_handle == NULL)
    {
        LI_FN(CloseClipboard).cached()();
        return;
    ...
    LI_FN(GlobalUnlock).cached()(wbuf_handle);
    LI_FN(EmptyClipboard).cached()();
    if (LI_FN(SetClipboardData).get()(CF_UNICODETEXT, wbuf_handle) == NULL)
        LI_FN(GlobalFree).cached()(wbuf_handle);
    LI_FN(CloseClipboard).cached()();
}
