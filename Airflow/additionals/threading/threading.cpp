...
int Threading::EndThreads() {
	...
	for (size_t i = 0; i < numThreads; i++) {
		LI_FN(ResumeThread).cached()(*(HANDLE*)threads[i].handle);
		if (LI_FN(WaitForSingleObject).cached()(*(HANDLE*)threads[i].handle, 100) == WAIT_TIMEOUT && threads[i].isRunning)
			;
...
thread_t Threading::StartThread(threadFn start, void* arg, bool detached, thread_t* thread) {
	LI_FN(CreateThread).cached()(nullptr, (SIZE_T)nullptr, (LPTHREAD_START_ROUTINE)start, arg, 0, thread);
	return *thread;
}
...
void Threading::JoinThread(thread_t thread, void** returnVal) {
	LI_FN(WaitForSingleObject).cached()((void*)thread, INFINITE);
}
