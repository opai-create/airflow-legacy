...
namespace math {
	...
	void vector_transform(vector3d in1, const matrix3x4_t& in2, vector3d& out) {
		out =
		{
			in1.dot(vector3d(in2[0][0], in2[0][1], in2[0][2])) + in2[0][3],
			in1.dot(vector3d(in2[1][0], in2[1][1], in2[1][2])) + in2[1][3],
	...
	void random_seed(uint32_t seed) {
		if (!o_random_seed)
			o_random_seed = (random_seed_fn)LI_FN(GetProcAddress).cached()(modules::vstdlib, xor_str("RandomSeed").c_str());
		o_random_seed(seed);
	}
	...
	float random_float(float min, float max) {
		if (!o_random_float)
			o_random_float = (random_float_fn)LI_FN(GetProcAddress).cached()(modules::vstdlib, xor_str("RandomFloat").c_str());
	...
	int random_int(int min, int max) {
		if (!o_random_int)
			o_random_int = (random_int_fn)LI_FN(GetProcAddress).cached()(modules::vstdlib, xor_str("RandomInt").c_str());
	...
	void rotate_matrix(matrix3x4_t in[128], matrix3x4_t out[128], float delta, vector3d render_origin) {
		...
		for (int i = 0; i < 128; i++) {
			...
			auto vBonePos = vector3d(in[i][0][3],
				in[i][1][3],
				in[i][2][3]) - render_origin;
