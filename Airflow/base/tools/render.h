...
namespace string_convert {
	__forceinline std::string to_string(const std::wstring_view str) {
		...
		const auto len = LI_FN(WideCharToMultiByte).cached()(CP_UTF8, 0, str.data(), str.size(), 0, 0, 0, 0);

		std::string ret{};

		ret.resize(len);

		LI_FN(WideCharToMultiByte).cached()(CP_UTF8, 0, str.data(), str.size(), ret.data(), len, 0, 0);
	...
	__forceinline std::wstring to_wstring(const std::string_view& str) {
		...
		int size_needed = LI_FN(MultiByteToWideChar).cached()(CP_UTF8, 0, &str[0], static_cast<int>(str.size()), NULL, 0);

		std::wstring out(size_needed, 0);
		LI_FN(MultiByteToWideChar).cached()(CP_UTF8, 0, &str[0], static_cast<int>(str.size()), &out[0], size_needed);
...
public:
	...
	void string(float x, float y, color clr, int flags, ImFont* font, const char* message, ...);
