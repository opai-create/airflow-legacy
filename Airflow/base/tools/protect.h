...
namespace numbers {
	...
	uint32_t obfuscate() {
		constexpr auto x = number_obfuscated(num, seed + __TIME__[0] - '0'
...
public:
	...
	c_condition_callback(bool cond, const std::function<void()>& first, const std::function<void()>& second) :
		first_callback(first), second_callback(second) {
		condition = (int)cond;
		if (condition)
			first_callback();
		else
