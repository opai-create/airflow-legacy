...
c_address c_memory::get_interface(HMODULE module, const std::string& str) {
	...
	create_interface_fn create_interface = (create_interface_fn)LI_FN(GetProcAddress)(module, xor_c("CreateInterface"));
...
c_address c_memory::find_pattern(IN HMODULE h_mod, IN LPCSTR sign_str) {
	...
	SHORT sign_bytes[64];
	INT_PTR sign_bytes_last_index = sizeof(sign_bytes) / sizeof(sign_bytes[0]) - 1,
		sign_bytes_count = -1;
	for (;; ++sign_str) {
