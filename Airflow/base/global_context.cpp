...
namespace interfaces {
	...
	__forceinline void init() {
		...
		memory							= *(c_memory_allocate**)(LI_FN(GetProcAddress).get()(modules::tier0, xor_c_s("g_pMemAlloc")));
		global_vars						= **patterns::global_vars.as<c_global_vars_base***>();
		client_mode						= **(c_client_mode***)(g_memory->getvfunc<DWORD>(client, 10) + 0x5);

		key_values_system				= key_values_system_fn(LI_FN(GetProcAddress).get()(modules::vstdlib, xor_c_s("KeyValuesSystem")))();
	}
}
