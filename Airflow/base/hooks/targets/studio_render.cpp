...
namespace tr {
	...
	namespace model_render {
		void __fastcall draw_model_execute(void* ecx, void* edx, void* ctx,
			...
			static std::string props = xor_str("props");

			if (std::strstr(info.model->name, props.c_str()))
				interfaces::render_view->set_blend(interfaces::render_view->get_blend() * (g_cfg.misc.prop_alpha * 0.01f));
