...
namespace tr::client {
	...
	void __fastcall frame_stage_notify(void* ecx, void* edx, int stage) {
		...
		g_animation_fix->on_net_update_end(stage);
		...
		if (stage == frame_render_start)
		{
			...
			for (auto& player : players)
			{
				...
				{
					g_local_animation_fix->on_update_anims(entity);
					continue;
				}

				g_animation_fix->on_update_anims(entity);
			}
		}
	...
	void __fastcall setup_clr_modulation(void* ecx, void* edx, int cnt, model_list_by_type_t* list) {
		...
		for (int i = 0; i < cnt; ++i) {
			...
			for (int j = 0; j < l.count; ++j) {
				...
				auto force_prop_alpha = [&]() {
					model->diffuse_modulation.h *= (g_cfg.misc.prop_alpha / 100.f);
				};
	...
	void __fastcall physics_simulate(c_csplayer* ecx, void* edx) {
		...
		g_tickbase->fix(ctx.cmd_number, ecx->tickbase());
	...
	bool __fastcall write_usercmd_to_delta_buffer(void* ecx, void* edx, int slot, void* buf, int from, int to, bool isnewcommand) {
		...
		return g_exploits->should_shift_cmd(&msg->new_commands, &msg->backup_commands, ecx, slot, buf, from, to);
	}
