...
namespace tr::player {
	...
	void __fastcall calc_viewmodel_view(void* ecx, void* edx, c_csplayer* owner, vector3d& eye_pos, vector3d& eye_angles) {
		...
		if (interfaces::client_state->delta_tick == -1 || g_ctx.uninject
			|| !owner->is_player() || !owner->is_alive() || !g_ctx.local || owner != g_ctx.local)
		...
		eye_pos += (fwd * g_cfg.misc.viewmodel_pos[1])
			+ (rt * g_cfg.misc.viewmodel_pos[0])
			+ (up * g_cfg.misc.viewmodel_pos[2]);
	...
	void __fastcall modify_eye_position(c_animstate* ecx, void* edx, vector3d& pos) {
		...
		auto player = (c_csplayer*)ecx->player;
		if (!player || !g_ctx.local || player != g_ctx.local)
			return original(ecx, edx, pos);
	}
