...
namespace tr {
		...
		void __fastcall packet_end(void* ecx, void* edx) {
			...
			if (clientstate->clock_drift_mgr.cur_clock_offset == clientstate->clock_drift_mgr.server_tick) {
				...
				auto correct = std::find_if(g_fake_lag->choked_ticks.begin(), g_fake_lag->choked_ticks.end(),
					[&ack_cmd](const choked_ticks_t& other_data) {
						return other_data.cmd == ack_cmd;
					}
				);
	...
	namespace engine {
		...
		bool __fastcall is_connected(void* ecx, void* edx) {
			...
			if (g_cfg.misc.unlock_inventory &&
				(uintptr_t)_ReturnAddress() == patterns::return_addr_loadout_allowed.as<uintptr_t>())
				return false;
		...
		void __vectorcall cl_move(float accumulated_extra_samples, bool final_tick) {
			...
			if (g_ctx.is_alive && g_exploits->recharging(g_ctx.cmd)) {
		...
		void __fastcall process_packet(c_netchan* ecx, void* edx, void* packet, bool header) {
			...
			for (auto it{ interfaces::client_state->events }; it != nullptr; it = it->next) {
				if (!it->class_id)
					continue;
