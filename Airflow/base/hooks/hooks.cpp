...
namespace hooks {
	__forceinline void hook_vmt() {
		...
		**reinterpret_cast<void***>(patterns::direct_reset.as<uintptr_t>()) = reinterpret_cast<void*>(&tr::direct::reset);
		...
		g_netvar_manager->hook_prop(__fnva1("DT_BaseViewModel"), __fnva1("m_nSequence"),
			(recv_var_proxy_fn)tr::netvar_proxies::viewmodel_sequence, original_sequence, true);
	}
	...
	__forceinline void hook_winapi() {
		if (g_ctx.window)
			g_ctx.backup_window = (WNDPROC)LI_FN(SetWindowLongA)(g_ctx.window, GWL_WNDPROC, (LONG_PTR)tr::wnd_proc);
	}
