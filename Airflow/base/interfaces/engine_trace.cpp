...
void c_engine_trace::trace_line(const vector3d& src, const vector3d& dst, int mask,
	c_handle_entity* entity, int collision_group, c_game_trace* trace) {
	static auto trace_filter_simple = patterns::trace_filter.add(0x3D).as<void*>();

	std::uintptr_t filter[4] = {
		*(std::uintptr_t*)(trace_filter_simple),
...
void c_engine_trace::trace_hull(const vector3d& src, const vector3d& dst, const vector3d& mins, const vector3d& maxs,
	int mask, c_handle_entity* entity, int collision_group, c_game_trace* trace) {
	static auto trace_filter_simple = patterns::trace_filter.add(0x3D).as<void*>();
