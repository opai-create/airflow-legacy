...
public:
	__forceinline c_trace_filter_simple()
		: vtable{ *patterns::trace_filter.add(0x3D).as<uintptr_t*>() } {
	}

	__forceinline c_trace_filter_simple(
		c_baseentity* const ignore_entity, const int collision_group
	) : vtable{ *patterns::trace_filter.add(0x3D).as<uintptr_t*>() },
		ignore_entity{ ignore_entity }, collision_group{ collision_group } {
	}
...
public:
	__forceinline c_trace_filter_skip_two_entities()
		: vtable{ *patterns::trace_filter_skip_entities.add(0x59).as<uintptr_t*>() } {
	}

	__forceinline c_trace_filter_skip_two_entities(
		c_baseentity* const ignore_entity0, c_baseentity* const ignore_entity1, const int collision_group = 0
	) : vtable{ *patterns::trace_filter_skip_entities.add(0x59).as<uintptr_t*>() },
		ignore_entity0{ ignore_entity0 }, collision_group{ collision_group }, ignore_entity1{ ignore_entity1 } {
	}
...
bool c_auto_wall::trace_to_exit(const vector3d& src, const vector3d& dir, const c_game_trace& enter_trace, c_game_trace& exit_trace) {
	...
	while (dist <= k_max_dist) {
		...
		if (exit_trace.start_solid
			&& exit_trace.surface.flags & surf_hitbox) {
			c_trace_filter_simple trace_filter{ exit_trace.entity, 0 };

			interfaces::engine_trace->trace_ray(
				{ out, src }, shot_hull,
				(i_trace_filter*)(&trace_filter), &exit_trace
			);
		...
		if (!exit_trace.did_hit()
			|| exit_trace.start_solid) {
			if (enter_trace.entity
				&& enter_trace.entity->index()
				&& this->is_breakable_entity(enter_trace.entity)) {
...
bool c_auto_wall::handle_bullet_penetration(c_csplayer* const shooter, const weapon_info_t* const wpn_data, const c_game_trace& enter_trace,
	vector3d& src, const vector3d& dir, int& pen_count, float& cur_dmg, const float pen_modifier) {
	...
	if (!this->trace_to_exit(enter_trace.end, dir, enter_trace, exit_trace)
		&& !(interfaces::engine_trace->get_point_contents(enter_trace.end, shot_hull) & shot_hull))
		return false;
	...
	if (enter_surface_data->game.material == 'G' || enter_surface_data->game.material == 'Y') {
	...
	else if (enter_trace.entity && cvars::ff_damage_reduction_bullets->get_float() == 0.f
		&& enter_surface_data->game.material == 'F' && ((c_csplayer*)enter_trace.entity)->team() == shooter->team()) {
		...
		combined_pen_modifier = (enter_surface_data->game.penetration_modifier
			+ exit_surface_data->game.penetration_modifier) * 0.5f;

		final_dmg_modifier = 0.16f;
	}

	if (enter_surface_data->game.material == exit_surface_data->game.material) {
		if (exit_surface_data->game.material == 'U'
			|| exit_surface_data->game.material == 'W')
			combined_pen_modifier = 3.f;
...
pen_data_t c_auto_wall::fire_bullet(c_csplayer* const shooter, c_csplayer* const target,
	const weapon_info_t* const wpn_data, const bool is_taser, vector3d src, const vector3d& dst, bool ignore_target) {
	...
	while (cur_dmg > 0.f) {
		...
		if (target) {
			if (trace.entity) {
				...
				if ((trace.entity == target || (is_player && ((c_csplayer*)trace.entity)->team() != shooter->team()))
...
pen_data_t c_auto_wall::fire_emulated(c_csplayer* const shooter, c_csplayer* const target, vector3d src, const vector3d& dst) {
	...
	while (cur_dmg > 0.f) {
		...
		if (trace.fraction == 1.f
			|| (trace.end - src).length(false) > max_dist)
			break;
		...
		if (trace.entity) {
			...
			last_hit_player =
				is_player ? static_cast<c_csplayer*>(trace.entity) : nullptr;
		}
		else
			last_hit_player = nullptr;
