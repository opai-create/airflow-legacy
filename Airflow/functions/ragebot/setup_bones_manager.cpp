				...
				for (int i = 0; i < layer_count; ++i) {
					...
					if (layer && layer->sequence > 1 && layer->weight > 0.f) {
						...
						if (!dispatch || layer->second_dispatch_sequence <= 0 || layer->second_dispatch_sequence >= (*(studio_hdr_t**)world_hdr)->local_seq)
							...
							if (ik_context)
								func_ptrs::add_dependencies(ik_context,
									*(float*)((uintptr_t)animating + 0xA14),
									layer->sequence, layer->cycle, poses.data(), layer->weight);

							world_setup->accumulate_pose(new_position, new_q, layer->second_dispatch_sequence,
								layer->cycle, layer->weight, time, world_ik);
		...
		for (int i = 0; i < this->layer_count; ++i) {
			...
			if (layer_count >= 0 && layer_count < this->layer_count) {
				...
				bone_setup->accumulate_pose(position, q,
					...
					final_layer->weight, time, ik_context);
			}
		}
	}
