...
namespace cheat_tools {
	...
	std::string hitbox_to_string(int id) {
		switch (id) {
		case 0:		return xor_c("head");				break;
		case 1:		return xor_c("neck");				break;
		case 2:		return xor_c("pelvis");				break;
		case 3:		return xor_c("stomach");			break;
		case 4:		return xor_c("lower chest");		break;
		case 5:		return xor_c("chest");				break;
		case 6:		return xor_c("upper chest");		break;
		case 7:		return xor_c("right thigh");		break;
		case 8:		return xor_c("left thigh");			break;
		case 9:		return xor_c("right leg");			break;
		case 10:	return xor_c("left leg");			break;
		case 11:	return xor_c("left foot");			break;
		case 12:	return xor_c("right foot");			break;
		case 13:	return xor_c("right hand");			break;
		case 14:	return xor_c("left hand");			break;
		case 15:	return xor_c("right upper arm");	break;
		case 16:	return xor_c("right lower arm");	break;
		case 17:	return xor_c("left upper arm");		break;
		case 18:	return xor_c("left lower arm");		break;
		}
	}

	std::string hitgroup_to_string(int hitgroup) {
		switch (hitgroup) {
		case hitgroup_generic:		return xor_c("generic");	break;
		case hitgroup_head:			return xor_c("head");		break;
		case hitgroup_chest:		return xor_c("chest");		break;
		case hitgroup_stomach:		return xor_c("body");		break;
		case hitgroup_leftarm:		return xor_c("leftarm");	break;
		case hitgroup_rightarm:		return xor_c("rightarm");	break;
		case hitgroup_leftleg:		return xor_c("leftleg");	break;
		case hitgroup_rightleg:		return xor_c("rightleg");	break;
		case hitgroup_gear:			return xor_c("gear");		break;
		case hitgroup_neck:			return xor_c("neck");		break;
		default:					return xor_c("unknown");
		}
	}
	...
	bool is_accuracy_valid(c_csplayer* player, point_t& point, float amount, float* out_chance) {
		...
		bool is_single_sniper = g_ctx.weapon->item_definition_index() == weapon_ssg08
			|| g_ctx.weapon->item_definition_index() == weapon_awp;

		const auto round_acc = [](const float accuracy) { return roundf(accuracy * 1000.f) / 1000.f; };

		if (g_ctx.spread < g_ctx.ideal_spread) {
			*out_chance = 0.5f;
			return true;
		}

		if (g_ctx.local->flags() & fl_ducking && !g_anti_aim->is_fake_ducking()) {
			if (round_acc(g_engine_prediction->predicted_inaccuracy) == round_acc(sniper ? weapon_info->inaccuracy_crouch_alt : weapon_info->inaccuracy_crouch)) {
				*out_chance = 0.5f;
				return true;
			}
		}
		else {
			if (round_acc(g_engine_prediction->predicted_inaccuracy) == round_acc(sniper ? weapon_info->inaccuracy_stand_alt : weapon_info->inaccuracy_stand)) {
				*out_chance = 0.6f;
				return true;
			}
		}

		if (!is_single_sniper && g_exploits->dt_enabled() && g_exploits->dt_bullet == 1) {
			*out_chance = amount / 2.5f;
			return true;
		}
...
namespace resolver {
	...
	__forceinline void store_freestanding(c_csplayer* player, records_t* current) {
		...
		freestanding.left_damage = g_auto_wall->fire_bullet(g_ctx.local, player, g_ctx.weapon_info, g_ctx.weapon->is_taser(),
			g_ctx.eye_position, left_eye_pos).dmg;

		freestanding.right_damage = g_auto_wall->fire_bullet(g_ctx.local, player, g_ctx.weapon_info, g_ctx.weapon->is_taser(),
			g_ctx.eye_position, right_eye_pos).dmg;
	...
	void start(c_csplayer* player, records_t* current) {
		...
		if (!current->on_ground) {
			...
			if (current->velocity.length(true) > 0.1f && !current->fake_walking) {
				player->eye_angles().y			= player->lby();

				resolver_info.last_moving_lby	= player->lby();
				...
				if (misses > 0) {
					...
					case 1:
						...
						break;
					case 2:
						player->eye_angles().y = at_target - 110.f;
						break;
					case 0:
						player->eye_angles().y = at_target + 110.f;
						break;
		...
		/*if (current->old_lby != player->lby()) {
			player->eye_angles().y	= player->lby();
			current->old_lby		= player->lby();
			current->lby_time		= player->simulation_time();
			current->lby_update		= true;
			resolver_info.mode		= xor_str("lby update");
			return;
		}

		if (current->lby_time + 1.1f <= player->simulation_time()) {
			player->eye_angles().y		= player->lby();
			current->old_lby			= player->lby();
			current->lby_time			= player->simulation_time();
			current->lby_update			= true;
