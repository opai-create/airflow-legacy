...
void c_utils::update_shot_cmd() {
	if (this->is_firing()) {
		...
		if (!g_cfg.binds[sw_b].toggled && !g_anti_aim->is_fake_ducking() && !g_exploits->cl_move.shift && !g_ctx.weapon->is_misc_weapon())
...
void c_utils::update_shot_time() {
	...
	if (g_anti_aim->is_fake_ducking() 
		|| g_ctx.weapon->is_pistols() 
		|| g_ctx.weapon->is_rifle() 
		|| g_ctx.weapon->is_misc_weapon() 
		|| g_exploits->hs_enabled()
		|| !g_cfg.antihit.enable) {
...
void c_utils::extrapolate(c_csplayer* player, vector3d& origin, vector3d& velocity, int& flags, bool on_ground) {
	...
	if (t.fraction != 1.f) {
		for (auto i = 0; i < 2; i++) {
			...
			if (dot < 0.f)
				velocity -= vector3d(dot * t.plane.normal.x,
...
void c_utils::on_net_update_end_after(int stage) {
	...
	g_engine_prediction->net_compress_apply(g_ctx.local->tickbase());
}
