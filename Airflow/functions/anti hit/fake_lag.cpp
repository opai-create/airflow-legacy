...
void c_fake_lag::bypass_choke_limit() {
	...
	if (!init) {
		...
		DWORD old_protect = 0;
		LI_FN(VirtualProtect).get()((void*)address, sizeof(uint32_t), PAGE_EXECUTE_READWRITE, &old_protect);
		*(uint32_t*)address = choke_clamp;
		LI_FN(VirtualProtect).get()((void*)address, sizeof(uint32_t), old_protect, &old_protect);
...
int c_fake_lag::get_choke_amount() {
	...
	if ((g_cfg.antihit.fakelag_conditions & 1) && standing
		...
		|| (g_cfg.antihit.fakelag_conditions & 4) && !g_utils->on_ground() && !landing
...
void c_fake_lag::on_predict_start()  {
	...
	if (choke_amount == 0 || !g_cfg.binds[sw_b].toggled && g_ctx.shooting && !g_anti_aim->is_fake_ducking() && !g_exploits->cl_move.shift) {
		if (!*g_ctx.send_packet)
			*g_ctx.send_packet = true;
		return;
	}
