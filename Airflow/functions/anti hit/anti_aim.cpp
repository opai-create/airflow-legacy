...
anti_aim_angles_t* c_anti_aim::get_config() {
	...
	bool stand = ground && (g_cfg.binds[sw_b].toggled || g_rage_bot->should_slide
		|| g_ctx.local->velocity().length(true) < 10.f);
...
bool c_anti_aim::is_peeking() {
	...
	for (auto& player : players) {
		...
		if (entity->dormant()) {
			vector3d poses[3]{
				origin,
				origin + entity->view_offset(),
				origin + vector3d(0.f, 0.f, entity->view_offset().z / 2.f)
			};
...
void c_anti_aim::automatic_edge() {
	...
	float left_damage = g_auto_wall->fire_bullet(player, g_ctx.local, weapon_info, weapon->is_taser(),
		shoot_pos, left_eye_pos).dmg;

	float right_damage = g_auto_wall->fire_bullet(player, g_ctx.local, weapon_info, weapon->is_taser(),
		shoot_pos, right_eye_pos).dmg;
...
void c_anti_aim::on_pre_predict() {
	if (g_ctx.local->move_type() == movetype_noclip
		|| g_ctx.local->move_type() == movetype_ladder)
		return;
...
void c_anti_aim::on_predict_start() {
	...
	bool stand = g_cfg.binds[sw_b].toggled || g_rage_bot->should_slide
		|| g_ctx.local->velocity().length(true) < 10.f;
	...
	auto real = [&]() {
		...
		switch (cfg->yaw) {
		case 1: g_ctx.cmd->viewangles.y += 180.f; break;
		case 2: {
	...
	if (g_cfg.antihit.desync) {
		if (!*g_ctx.send_packet) {
			...
			bool lby_update = stand
				&& !interfaces::client_state->choked_commands
				&& interfaces::global_vars->cur_time >= g_local_animation_fix->local_info.last_lby_time;
