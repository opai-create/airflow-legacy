...
inline bool is_dir(const TCHAR* dir) {
	uintptr_t flag = LI_FN(GetFileAttributesA)(dir);
	if (flag == 0xFFFFFFFFUL) {
		if (LI_FN(GetLastError)() == ERROR_FILE_NOT_FOUND)
			return false;
	}
...
namespace config {
	...
	void create_config_folder() {
		...
		if (SUCCEEDED(LI_FN(SHGetFolderPathA).get()(NULL, CSIDL_PERSONAL, NULL, 0, path)))
			folder = std::string(path) + xor_c("\\airflow\\");

		if (!is_dir(folder.c_str()))
			LI_FN(CreateDirectoryA).get()(folder.c_str(), NULL);
	}
	...
	void create(const std::string& config_name) {
		...
		auto& rage = json_obj[xor_c("rage")]; {
			...
			save_bool(rage, xor_c("jitter_fix"), default_config.rage.jitter_fix);
			...
			for (int i = 0; i < weapon_max; i++) {
				...
				save_int(rage[sub], xor_c("target_selection"), default_config.rage.weapon[i].target_selection);
	...
	void save(const std::string& config_name) {
		...
		auto& rage = json_obj[xor_c("rage")]; {
			...
			save_bool(rage, xor_c("jitter_fix"), g_cfg.rage.jitter_fix);
			...
			for (int i = 0; i < weapon_max; i++) {
				...
				save_int(rage[sub], xor_c("target_selection"), g_cfg.rage.weapon[i].target_selection);
	...
	void load(const std::string& config_name) {
		...
		auto& rage = json_obj[xor_c("rage")]; {
			...
			load_bool(rage, xor_c("jitter_fix"), g_cfg.rage.jitter_fix);
			...
			for (int i = 0; i < weapon_max; i++) {
				...
				load_int(rage[sub], xor_c("target_selection"), g_cfg.rage.weapon[i].target_selection);
