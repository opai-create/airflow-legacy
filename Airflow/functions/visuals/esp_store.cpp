...
namespace esp_renderer {
    void esp_strings(esp_box_t& box, esp_objects_t& object, int max_bar_width, float& offset) {
        ...
        auto render_string = [&](float x, float y, bool center = false) {
            auto fl = object.dropshadow ? dropshadow_ : outline_;

            g_render->string(x, y, object.color.new_alpha(object.color.a() * object.custom_alpha), center ? centered_x | fl : fl,
                get_current_font(object.font_type), object.string.c_str());
        };
    ...
    void esp_bars(esp_box_t& box, esp_objects_t& object, int& offset) {
        ...
        const auto outline_color = color(0, 0, 0, object.color.a() * object.custom_alpha);
        ...
        case esp_left: {
            ...
            if (object.bar < object.bar_max)
                g_render->string((box.x - (bar_width * 2.f)) - add + 1.f, (box.y + bar_h) - 1.f, text_color, centered_x | outline_, g_fonts.pixel, value.c_str());
        } break;
        case esp_up: {
...
esp_box_t get_bounding_box(c_baseentity* entity) {
    ...
    if (entity->is_player()) {
        ...
        auto mins = player->bb_mins();
        auto maxs = player->bb_maxs();

        auto abs_origin = player->get_abs_origin();
        auto center = abs_origin + math::interpolate(mins, maxs, 0.5f);

        float stand_pose = player->pose_parameter()[1];
        float body_pitch = player->pose_parameter()[12];
