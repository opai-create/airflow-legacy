...
void render_nade_range(float radius, float timer, const vector3d& origin, const color& clr, const char* icon, float anim, float anim2, float alpha) {
	...
	g_render->draw_list->AddConvexPolyFilled(points.data(), points.size(),
		clr.new_alpha(clr.a() * 0.8f * alpha).as_imcolor());

	g_render->draw_list->AddPolyline(points.data(), points.size(),
		clr.new_alpha(clr.a() * alpha).as_imcolor(),
		true, 2.f);
	...
	g_render->filled_triangle(positions[0].x, positions[0].y,
		positions[1].x, positions[1].y,
		positions[2].x, positions[2].y, color(255, 255, 255, 255 * alpha));

	g_render->triangle(positions[0].x, positions[0].y,
		positions[1].x, positions[1].y,
		positions[2].x, positions[2].y, color(0, 0, 0, 100 * alpha), 2.f);
}
