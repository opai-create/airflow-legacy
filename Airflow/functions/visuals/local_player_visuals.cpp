...
void c_local_visuals::thirdperson() {
	...
	if (interfaces::input->camera_in_third_person) {
		...
		auto origin = fd
			? g_ctx.local->get_render_origin() + vector3d(0.0f, 0.0f, interfaces::game_movement->get_player_view_offset(false).z + 0.064f)
			: g_ctx.local->get_eye_position();
	...
	if (g_ctx.in_game) {
		...
		if (g_ctx.weapon) {
			...
			if (!g_ctx.is_misc && g_cfg.misc.pen_xhair) {
				...
				auto awall = g_auto_wall->fire_bullet(g_ctx.local, nullptr, g_ctx.weapon_info, g_ctx.weapon->is_taser(), start,
					start + (dir * g_ctx.weapon_info->range), true);
...
void c_local_visuals::on_directx() {
	...
	if (!peek_positions.empty()) {
		...
		color clr = first_clr.multiply(second_clr, g_movement->peek_move).new_alpha(std::lerp(first_clr.a(), second_clr.a(), (float)g_movement->peek_move));
