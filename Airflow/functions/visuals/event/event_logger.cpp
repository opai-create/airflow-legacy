...
void c_event_logger::add_message(const std::string& text, int message_type, bool debug) {
	...
	if (!debug) {
		...
		interfaces::convar->print_console_color(prefix.second, xor_c("%s "), prefix.first.c_str());
		interfaces::convar->print_console_color(prefix.second, "%s \n", text.c_str());
	}
	else
		interfaces::convar->print_console_color(color(220, 220, 220), "%s \n", text.c_str());
}
...
void c_event_logger::on_directx() {
	...
	for (int i = messages.size() - 1; i >= 0; --i) {
		...
		if (messages[i].alpha > 0.f) {
			...
			// log body
			g_render->blur(position.x, position.y, log_size.x, log_size.y, color(130, 130, 130, 255 * messages[i].alpha), 4.f);
			...
			// log prefix
			g_render->string(position.x + 12.f, position.y + 4.f,
				color(255, 255, 255, messages[i].alpha * 255), 0, g_fonts.main, prefix.first.c_str());
			...
			// main log
			g_render->string(separator_pos + 10.f, position.y + 4.f, color(255, 255, 255, 120.f * messages[i].alpha),
				0, g_fonts.main, messages[i].text.c_str());
		}
