	...
	struct nade_path_t {
		...
		__forceinline bool physics_simulate() {
			...
			const auto move = vector3d(
				nade_velocity.x * interfaces::global_vars->interval_per_tick,
				nade_velocity.y * interfaces::global_vars->interval_per_tick,
				(nade_velocity.z + new_velocity_z) / 2.f * interfaces::global_vars->interval_per_tick
			);
		...
		__forceinline void physics_trace_entity(const vector3d& src, const vector3d& dst, std::uint32_t mask, trace_t& trace) {
			interfaces::engine_trace->trace_hull(
				src, dst, { -2.f, -2.f, -2.f }, { 2.f, 2.f, 2.f },
				mask, nade_owner, cur_collision_group, &trace
			);
			...
			if (trace.start_solid
				...
				interfaces::engine_trace->trace_hull(
					src, dst, { -2.f, -2.f, -2.f }, { 2.f, 2.f, 2.f },
					mask & ~contents_current_90, nade_owner, cur_collision_group, &trace
				);
			}

			if (!trace.did_hit()
				|| !trace.entity
				|| !((c_csplayer*)trace.entity)->is_player())
				return;
		...
		__forceinline void physics_push_entity(const vector3d& push, trace_t& trace) {
			...
			if (trace.start_solid) {
				...
				interfaces::engine_trace->trace_line(
					nade_origin - push, nade_origin + push,
					(mask_solid | contents_current_90) & ~contents_monster,
					nade_owner, cur_collision_group, &trace
				);
			}
			...
			if (((c_csplayer*)trace.entity)->is_player()
				|| nade_idx != weapon_tagrenade && nade_idx != weapon_molotov && nade_idx != weapon_incgrenade)
				return;

			if (nade_idx != weapon_tagrenade
				&& trace.plane.normal.z < std::cos(math::deg_to_rad(cvars::weapon_molotov_maxdetonateslope->get_float())))
				return;
		...
		__forceinline void predict_nade(const vector3d& origin, const vector3d& velocity, float throw_time, int offset) {
			...
			switch (nade_idx) {
			case weapon_smokegrenade: next_think_tick = math::time_to_ticks(1.5f); break;
			case weapon_decoy: next_think_tick = math::time_to_ticks(2.f); break;
			case weapon_flashbang:
			case weapon_hegrenade:
		...
		int nade_idx{}, nade_tick_count{}, next_think_tick{},
			last_update_tick{}, bounces_count{};
