...
void render_main_esp(c_esp_store::player_info_t* info) {
	...
	if (enemy_esp.elements & 64) {
		...
		if (flags.armorvalue) {
			info->objs[iter++] = esp_objects_t{ true, esp_right, color(255, 255, 255),
				0.f, 0.f, flags.hashelmet ? xor_str("HK") : xor_str("K"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.zoom) {
			info->objs[iter++] = esp_objects_t{ true, esp_right, color(48, 196, 255),
				0.f, 0.f, xor_str("ZOOM"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.aimtarget) {
			info->objs[iter++] = esp_objects_t{ true, esp_right, color(255, 255, 255),
				0.f, 0.f, xor_str("HIT"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.fakeduck) {
			info->objs[iter++] = esp_objects_t{ true, esp_right,  color(255, 69, 74),
				0.f, 0.f, xor_str("FD"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.deffensive) {
			info->objs[iter++] = esp_objects_t{ true, esp_right,  color(255, 255, 255),
				0.f, 0.f, xor_str("EX"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.reloading) {
			info->objs[iter++] = esp_objects_t{ true, esp_right,  color(184, 199, 255),
				0.f, 0.f, xor_str("RELOAD"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.havebomb) {
			info->objs[iter++] = esp_objects_t{ true, esp_right,  color(255, 147, 128),
				0.f, 0.f, xor_str("C4"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.havekit) {
			info->objs[iter++] = esp_objects_t{ true, esp_right,  color(125, 212, 255),
				0.f, 0.f, xor_str("KIT"), font_pixel, false, info->dormant_alpha };
		}
		if (flags.defusing) {
			info->objs[iter++] = esp_objects_t{ true, esp_right,  color(201, 125, 255),
				0.f, 0.f, xor_str("DEF"), font_pixel, false, info->dormant_alpha };
		}
	}
