	...
	if (predicted_nades.find(handle) == predicted_nades.end()) {
		predicted_nades.emplace(
			...
			std::forward_as_tuple(
				((c_basecombatweapon*)entity)->thrower(),
				get_weapon_index(client_class->class_id),
				entity->origin(),
				((c_csplayer*)entity)->velocity(),
				entity->grenade_spawn_time(),
...
void c_grenade_warning::calc_local_nade_path() {
	...
	interfaces::engine_trace->trace_hull(
		...
		mask_solid | contents_current_90, g_ctx.local, collision_group_none, &trace
	);
	...
	local_path.predict_nade(
		trace.end - direction * 6.f,
		direction * (std::clamp< float >(
			g_ctx.weapon_info->throw_velocity * 0.9f, 15.f, 750.f
			) * (throw_strength * 0.7f + 0.3f)) + velocity * 1.25f,
...
void c_grenade_warning::on_directx() {
	...
	for (auto& l : list) {
		...
		if (data.last_path_pos.x != 0.f && data.last_path_pos.y != 0.f) {
			...
			g_render->string(weapon_icon_pos.x, weapon_icon_pos.y, color(255, 255, 255, 255 * mod),
				centered_x | dropshadow_, g_fonts.weapon_icons_large, data.preview_icon.c_str());

			g_render->string(weapon_icon_pos.x, weapon_icon_pos.y + weapon_icon_size.y + 12.f, color(255, 255, 255, 255 * mod),
				centered_x | outline_, g_fonts.pixel, data.preview_name.c_str());
