...
class c_chams {
private:
	chams_t safe_left_chams = {
		true,
		true,
		3,
		100,
		5,
		c_float_color(255, 0, 0, 100),
		c_float_color(255, 0, 0, 100)
	};
	...
	struct hook_data_t {
		...
		__forceinline void init(draw_model_execute_fn original, void* ecx, void* edx, void* ctx,
	...
	std::array<c_material*, 7>		materials_list{};
...
public:
	...
	void init_materials();
	void on_draw_model_execute(draw_model_execute_fn original, void* ecx, void* edx, void* ctx,
		const draw_model_state_t& state, const model_render_info_t& info, matrix3x4_t* bone_to_world);
